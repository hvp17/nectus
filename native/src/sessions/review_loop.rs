//! Task review prompt + verdict helpers shared by the inline `/review` subagent
//! path (`sessions::spawn_inline_review` → `review_runtime::run_inline_review`).
//! The headless ACP review driver itself lives in `review_runtime.rs`.

use super::verdict::VerdictToken;
use crate::models::{ReviewVerdict, TaskSummary};

pub(super) const UNCLEAR_REVIEW_ERROR: &str = "Reviewer output did not include a clear verdict";
