...
#[test]
fn task_repos_for_tasks_batches_and_groups_memberships() {
    let db = Database::open_in_memory().unwrap();
    let alpha = insert_workspace_test_repo(&db, "alpha");
    let beta = insert_workspace_test_repo(&db, "beta");
    let task_one = db
        .create_task_record(alpha, "One".into(), None, None, false, None)
        .unwrap();
    let task_two = db
        .create_task_record(alpha, "Two".into(), None, None, false, None)
        .unwrap();
    db.conn
        .execute(
            "INSERT INTO task_repos (task_id, repo_id, branch_name, worktree_path, pr_url, position)
             VALUES (?1, ?2, 'feat/two', NULL, NULL, 1)",
            params![task_two.id, beta],
        )
        .unwrap();

    let grouped = db
        .task_repos_for_tasks(&[task_one.id, task_two.id])
        .unwrap();

    assert_eq!(grouped.get(&task_one.id).unwrap().len(), 1);
    let task_two_repos = grouped.get(&task_two.id).unwrap();
    assert_eq!(task_two_repos.len(), 2);
    assert_eq!(task_two_repos[0].repo_id, alpha);
    assert_eq!(task_two_repos[1].repo_id, beta);
}
