...
| `session_chat` | ACP chat update: task id, chat session id, optional agent profile id, normalized `ChatMessage`, and `done` flag. Streaming updates are cache-only in the frontend; settled user/agent turns are persisted to `chat_messages`. | `native/src/sessions/acp_manager.rs` |
