...
- Frontend infrastructure (the three layers the shell composes):
  - `src/queries/`: the **TanStack Query** server-state layer — `queryClient.ts` (per-mount client factory), `keys.ts` (the key catalog), `cache.ts` (`makeCacheSetter`, a `setState`-shaped writer backed by the cache), `core.ts` (bootstrap read hooks: repos/workspaces/tasks/agentProfiles/settings), and per-domain hooks (`github.ts`, …). All server/command reads and cache writes (including from the Tauri event handlers) go through here, not component state.
  - `src/store/`: the **Zustand** UI/runtime store — `appStore.ts` composes concern-split slices (`navigation`, `selection`, `runtime`, `sessionRuntime`, `notification`); `setState.ts` makes setters `SetStateAction`-compatible drop-ins for `useState`. Owns the shell state that is NOT server state: `currentView`/`activeWorkspaceId`, selection (repo/task/agent), the push-driven `liveLines`/`taskAttention`, `message`/`taskToast`, and the global `busy` flag. (The New Task composer draft stays in its own cohesive `useCreateTaskForm` hook; per-domain selection like the open PR-review/JIRA item lives in those domain hooks.)
  - `src/AppRouter.tsx`: the **TanStack Router** shell — `AppLayout` (root route) owns the single `useApp()` instance, the app-level handlers (navigate/openTask/openCreateTaskModal), and a one-directional store→router sync (memory history) that drives `<Outlet/>` from the store's `currentView`; routed leaf views (`/` mission, `/board`, `/workspace`, `/settings`, `/reviews`, `/jira`) read `useApp` via a context. The composer / workspace manager / open task overlay the viewport ahead of the routed view. (View navigation is now one tick async — tests assert post-navigation content with `findBy*`/`waitFor`.)
- `src/App.tsx`: thin root that mounts the app-wide providers (`QueryClientProvider` + `TooltipProvider` + `RouterProvider`) with a fresh `QueryClient` and router per mount for test isolation; all shell/routing lives in `src/AppRouter.tsx`
...
- `src/hooks/useApp.ts`: orchestration **façade** that composes the server-state layer (`src/queries/*`) and the UI/runtime store (`src/store/*`) plus the per-domain hooks, returning the flat surface the shell/views consume. Server reads come from the query cache via the `set*` cache-setter shims (so mutation/event call sites keep `setState`-style signatures); `refresh()` is `invalidateQueries` and the default-selection logic (default agent/repo, drop-deleted-workspace) runs in reactive effects. `activeWorkspaceId` is the **focused** workspace (drives the workspace board content), not a global scope filter — Mission Control always shows all projects. The composer's cross-repo mode is driven by a separate `newTaskWorkspaceId` (the New Task scope toggle): seeded from `activeWorkspaceId` when opened from a workspace board, but independently switchable to any eligible workspace from any entry point. Exports `workspaceBoardTasks` (tasks in the focused workspace) and `openWorkspaceBoard` (sets focus + routes to `workspace` view); workspace CRUD
