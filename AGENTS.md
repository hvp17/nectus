...
- Update `docs/jira-integration.md` when the JIRA connection model (API token / REST endpoints / `acli` fallback), the board JQL/columns, work-item management, or task↔story link behavior changes.
...
- `docs/jira-integration.md`: the token-primary connection model (API token first, `acli` fallback, REST-first dispatch), JQL board, status columns, work-item management, and local task↔story links.
...
- `native/src/jira.rs`: `acli` (Atlassian CLI) integration — the **fallback** JIRA connection (the API token in `jira_rest.rs` is primary; `jira_rest_or_acli` in `lib.rs` dispatches REST-first per command). Owns the acli shell-outs for connection status, project list, work-item search/view/create/transition/assign/comment, the **shared** tolerant JSON parsing (`parse_work_items`/`parse_work_item`/`parse_projects` also parse the raw REST payloads — `/search/jql` pages, `/project/search`, Agile issues), the structured-config JQL builder (`build_board_jql`, incl. the `status in (...)` and `parent = "<epic>"` filter clauses, so the UI never types JQL), the epic-picker helpers (`build_epics_jql`, `list_epics`), and the create argument builder + new-key parser (`build_create_args`, `parse_created_key`); no OAuth, no stored tokens
- `native/src/jira_rest.rs`: the **primary** JIRA Cloud REST layer (`ureq`, Basic auth), gated on a user API token. Core-API parity with acli — `list_projects` (`GET /project/search`), the paginated `search` (`POST /search/jql` + `nextPageToken`; the old `/search` endpoint was removed by Atlassian), `view` (`GET /issue/{key}`), `assign` (`PUT .../assignee` with `@me`/email/display-name → account-id resolution via `/myself` + `/user/search`, `pick_account_id`), `comment`/`create` (plain text wrapped in minimal ADF by `text_to_adf`, the inverse of `jira::collect_adf_text`) — plus the fixture-tested parsers `parse_transitions` / `parse_project_statuses` (with `verify` (`/myself`), `list_transitions`, `project_statuses`, `perform_transition`) **and** the Agile-API sprint layer under `/rest/agile/1.0` (`find_scrum_board`, `parse_sprints`, and the `sprint_board` orchestrator that returns active/future sprints + backlog as `JiraSprintLane`s, issues carrying their epic). Also powers the token-only features: the legal-transition dropdown, the board status filter, all status columns (incl. empty), and Sprint view
...
reviews, and PTY sessions. Rust emits ten events — `session_output`,
`session_activity`, `session_idle`, `session_needs_input`, `session_exited`,
`review_loop_updated`, `review_output`, `pr_review_output`, `pr_review_updated`,
and `diagnostic_log` (one line per captured backend `tracing` record, for the
...
- Frontend infrastructure (the three layers the shell composes):
  ...
  - `src/hooks/useEventBridge.ts`: the single, **mount-once** Tauri event bridge (mounted in `AppLayout`). It owns every session/review/PR subscription (`session_activity`/`idle`/`needs_input`/`exited`, `review_loop_updated`, `pr_review_updated`) and routes each event to the query cache (tasks, review loop/runs, PR reviews) or the Zustand store (`liveLines`/`taskAttention`/toast/message). Each channel delegates the Tauri `listen` lifecycle to `useTauriEvent`, so subscriptions are independent and share the same late-unlisten/error handling. `session_exited` resolves its task via `activeSessionId` (the payload has no taskId). Because events are centralized, the domain hooks are pure cache consumers callable per-component. `review_output` (per-component live stream) stays in `useTaskReviewLoop` and `pr_review_output` likewise in `usePrReviews` (the selected single review's live stdout); `useTaskDiff` keeps its own `session_idle` (mounted once); `TerminalPane` keeps `session_output`.
...
- `src/components/ReviewTerminalPane.tsx`: read-only xterm.js pane that renders a reviewer's live stdout (and its last recorded output between runs); no input, session, or snapshot. Shared by the task workspace Review tab and the single PR review's Terminal view in `PrReviewDetail`
...
- Codex, Claude, Antigravity, OpenCode, and custom agents are launched as configurable CLI commands.
...
- JIRA is **token-primary**: the recommended connection is a user-pasted JIRA Cloud API token kept in the macOS Keychain (`native/src/jira_secret.rs`), never in SQLite, and never an OAuth flow — the **one** deliberate exception to the "no app-managed tokens" default; don't broaden token storage beyond this. Every JIRA command dispatches REST-first and degrades to `acli` (`jira_rest_or_acli` in `native/src/lib.rs`); keep `acli` working as the token-free fallback rather than a hard dependency.
