...
- `native/src/github.rs`: `gh` CLI integration — connection status, pull request detect/status parsing (incl. the per-check GitHub Actions/CI drill-down parsed from one `gh pr view` `statusCheckRollup`), and `comment_on_pull_request` for posting a review back (no OAuth, no stored tokens). The write helpers (`create_pull_request`, `merge_pull_request` squash/merge/rebase — no `--delete-branch`, `set_pull_request_ready`, `close_pull_request`) are **dormant**: still compiled, tested, and registered, but no longer reached by the UI — PR writes are now agent-driven (see `src/hooks/useGithubShipActions.ts`)
...
- Frontend infrastructure (the three layers the shell composes):
  ...
  - `src/AppRouter.tsx`: the **TanStack Router** shell — `AppLayout` (root route) composes the shell's data directly from queries + the store (there is no `useApp` god-hook anymore), runs `useEventBridge` + `useShellBootstrap` once, and drives a one-directional store→router sync (memory history) that renders `<Outlet/>` from the store's `currentView`. The routed leaf views (`/` mission, `/board`, `/workspace`, `/settings`, `/reviews`, `/jira`) and the overlay components (`ComposerOverlay`, `TaskWorkspaceOverlay`, the workspace manager) are **self-sufficient** — each reads its own queries/store/domain-hooks; only the cross-cutting `openTask` / `openCreateTaskModal` / `appUpdate` flow through a small React context. (View navigation is now one tick async — tests assert post-navigation content with `findBy*`/`waitFor`.)
