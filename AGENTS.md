...
- Agent runtime: ACP chat over stdio; reviewer output renders in a read-only
  `xterm.js` pane
...
Keep OS, git, SQLite, ACP child-process behavior, and reviewer CLI behavior in Rust.
...
`review_output`, `session_chat`, `session_chat_usage`, `chat_session_exited`,
`pr_review_output`, `pr_review_updated`, and `diagnostic_log` (one line per
