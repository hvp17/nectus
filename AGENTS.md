...
- `src/App.tsx`: sidebar shell, view routing (`mission` | `board` | `workspace` | `jira` | `reviews` | `settings`), and top-level composition; the `workspace` route renders the aggregated kanban (`Workspace.tsx`) scoped to the focused workspace's repos
...
- Release flow: bump the version to the same `X.Y.Z` in `native/tauri.conf.json`, `package.json`, and `native/Cargo.toml`; commit; `git tag vX.Y.Z`; `git push origin vX.Y.Z`. The `v*` tag triggers `.github/workflows/release.yml` (macos-latest/arm64, `tauri-apps/tauri-action@v0` with `GITHUB_TOKEN` + the `TAURI_SIGNING_PRIVATE_KEY`/`TAURI_SIGNING_PRIVATE_KEY_PASSWORD` repo secrets), which builds aarch64, signs the updater artifacts, and auto-publishes a Release with the `.dmg`, `.app.tar.gz`, `.sig`, and `latest.json` (shape: `{ version, notes, pub_date, platforms: { "darwin-aarch64": { signature, url } } }`). Out of scope: Apple notarization, Windows/Linux/Intel, background polling beyond the launch check.
